...
import {
  Bell, Box, BarChart3, CalendarClock, CreditCard, ExternalLink, FileCheck2,
  Mail, MessageSquare, Package, ShieldCheck, Truck, UserCircle,
} from "lucide-react";

import { getSupabaseBrowserClient } from "@/lib/supabase/browser";
import { useCustomerSession, CustomerShell } from "@/components/dashboard/customer-shell";
...
export function CustomerSettings() {
  const { data, state, errorMessage, theme, setTheme, messages, setMessages, bookings, signOut } = useCustomerSession();
  const profile = data?.profile ?? null;
  const unreadCount = messages.filter((m) => !m.read_at && m.direction === "outbound").length;
  const upcomingBookingsCount = bookings.filter((b) => new Date(b.start_time) >= new Date()).length;
  ...
  function openNotifications() {
    setMessages((prev) =>
      prev.map((m) => (m.direction === "outbound" && !m.read_at ? { ...m, read_at: new Date().toISOString() } : m)),
    );
          ...
          <Card>
            ...
            <CardContent>
              <label className="space-y-1 text-sm font-medium">
                ...
                <Select value={theme} onValueChange={(v) => { setTheme(v as "light" | "dark"); showToast("Appearance preference saved."); }}>
        ...
        <Card>
          ...
          <CardContent className="grid gap-2 sm:grid-cols-2">
            {[
              ...
              { label: "Analytics", href: "/dashboard/customer/analytics", icon: BarChart3 },
