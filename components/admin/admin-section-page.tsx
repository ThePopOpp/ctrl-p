...
import { adminSectionConfigs } from "@/lib/admin/section-configs";
        ...
        <header className="sticky top-0 z-10 border-b bg-background/90 backdrop-blur lg:pl-[238px]">
          <div className="flex h-12 items-center gap-3 px-5">
            <div className="hidden items-center gap-2 text-xs text-muted-foreground md:flex">
              ...
              <span className="font-medium text-foreground">{config.eyebrow}</span>
            </div>
            <div className="ml-auto flex items-center gap-2">
              <div className="relative hidden w-[380px] md:block">
                <Search className="absolute left-3 top-2 h-4 w-4 text-muted-foreground" />
                <Input className="h-8 rounded-lg pl-9 text-xs" placeholder={config.searchPlaceholder} />
              </div>
            ...
            <>
              ...
              <section className="grid gap-4 xl:grid-cols-3">
                {config.panels.map((panel) => (
                  <WorkflowCard key={panel.title} title={panel.title} description={panel.description}>
