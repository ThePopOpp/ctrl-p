    ...
    <div className={cn(theme === "dark" && "dark")}>
      <div className="min-h-screen bg-background text-foreground">
        <aside className="fixed inset-y-0 left-0 z-20 hidden w-[238px] border-r bg-card/95 px-3 py-3 lg:block">
          <div className="mb-4 flex items-center gap-3 px-2"><img src="/logos/app-icon.svg" alt="ctrl+p" className="h-9 w-9 rounded-lg" /><div><div className="text-[10px] font-semibold uppercase tracking-[0.32em] text-muted-foreground">controlp.io</div><div className="text-sm font-semibold">Super Admin</div></div></div>
...
function CreateOrderSheet({ open, onOpenChange, data, onCreated }: { open: boolean; onOpenChange: (open: boolean) => void; data: AdminDashboardData | null; onCreated: (orderId: string) => Promise<void> }) {
  ...
  async function create() {
    ...
    try {
      const order = await createAdminOrder({ userId: userId === "manual" ? undefined : userId, productId, quantity: Number(quantity || 1), unitPrice: Number(unitPrice || 0), status, paymentStatus, productionStatus, company, customerEmail: email, customerPhone: phone, customerNotes, internalNotes, dueAt, shippingMethod });
